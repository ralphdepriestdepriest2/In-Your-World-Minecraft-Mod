				...
				if (world.canSeeSkyFromBelowWater(BlockPos.containing(x, y, z))) {
					if (!(!world.getEntitiesOfClass(VoladorEntity.class, AABB.ofSize(new Vec3(x, y, z), 4000, 4000, 4000), e -> true).isEmpty())) {
						if (world instanceof ServerLevel _level) {
							Entity entityToSpawn = InYourWorldModEntities.VOLADOR.get().spawn(_level, BlockPos.containing(x + Mth.nextDouble(RandomSource.create(), 20, 45), y + Mth.nextDouble(RandomSource.create(), 10, 40), z),
