			...
			if (InYourWorldModVariables.MapVariables.get(world).tiempoSpawn2 == 0) {
				if (!(!world.getEntitiesOfClass(EntidadStalkerEntity.class, AABB.ofSize(new Vec3(x, y, z), 4000, 4000, 4000), e -> true).isEmpty())) {
					if (world instanceof ServerLevel _level) {
						Entity entityToSpawn = new EntidadStalkerEntity(InYourWorldModEntities.ENTIDAD_STALKER.get(), _level);
						entityToSpawn.moveTo((x + Mth.nextDouble(RandomSource.create(), 1, (double) ConfiguracionConfiguration.STALKERMAXPROBABILITY.get())), y, z, world.getRandom().nextFloat() * 360F, 0);
