...
public class Falso2TimerProcedure {
	@SubscribeEvent
		...
		if (ConfiguracionConfiguration.SPAWNESTRUCTURAFALSA2.get()) {
			if (InYourWorldModVariables.MapVariables.get(world).estructuraSpawn2 == 0) {
				if (world.canSeeSkyFromBelowWater(BlockPos.containing(x, y, z))) {
					if (!(!world.getEntitiesOfClass(EstructuraFalsa2Entity.class, AABB.ofSize(new Vec3(x, y, z), 4000, 4000, 4000), e -> true).isEmpty())) {
						if (world instanceof ServerLevel _level) {
							Entity entityToSpawn = InYourWorldModEntities.ESTRUCTURA_FALSA_2.get().spawn(_level, BlockPos.containing(x + Mth.nextDouble(RandomSource.create(), 80, 110), y + Mth.nextDouble(RandomSource.create(), 80, 125), z),
