...
public class LogroStalkerProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
			...
			return;
		{
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof EntidadStalkerEntity && entity instanceof Player) {
					if (entity instanceof ServerPlayer _player) {
						Advancement _adv = _player.server.getAdvancements().getAdvancement(new ResourceLocation("in_your_world:behind"));
